  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#fff8ef_0,#f7efe4_38%,#f4eee7_100%)] text-[#1f1b17]">
      ...
      <section className="mx-auto w-[min(1400px,calc(100%-32px))] py-8">
        ...
        {loading ? (
          <div className="rounded-[26px] border border-dashed border-[#dccdbb] bg-white/70 px-6 py-14 text-center text-[#6f6254] shadow-sm">Chargement...</div>
        ) : (
            ...
            <div className="flex-1 min-w-0">
              <div className={cardClass + " p-5 md:p-7"}>
                ...
                {activeTab === "salon" && (
                  <div>
                    <div className="mb-6">
                      <div className="mb-2 flex items-center gap-2 text-sm font-bold text-[var(--gold)]"><span className="text-xl">🏪</span> Salon</div>
                      <h2 className="text-2xl font-black tracking-tight">Téléphone, horaires et jours d'ouverture</h2>
                    </div>
                    {settings ? (
                      <div>
                ...
                {activeTab === "fermetures" && (
                  <div>
                    <div className="mb-6">
                      <div className="mb-2 flex items-center gap-2 text-sm font-bold text-[var(--gold)]"><span className="text-xl">📆</span> Planning</div>
                      <h2 className="text-2xl font-black tracking-tight">Fermetures exceptionnelles</h2>
                    </div>
                        ...
                        <div className="grid gap-4">
                          ...
                          {!newClosureAllDay && (
                            <div className="grid gap-4 md:grid-cols-2">
                              <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Début<input type="time" value={newClosureStartTime} onChange={(e) => setNewClosureStartTime(e.target.value)} className={fieldClass} /></label>
                              <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Fin<input type="time" value={newClosureEndTime} onChange={(e) => setNewClosureEndTime(e.target.value)} className={fieldClass} /></label>
                            </div>
                          )}
                          <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">
                            Motif
                            <input type="text" value={newClosureReason} onChange={(e) => setNewClosureReason(e.target.value)} placeholder="Formation, congé..." className={fieldClass} />
                          </label>
                        </div>
                        <div className="mt-5 flex justify-end">
                          <button type="button" onClick={handleCreateClosure} disabled={savingClosure} className={primaryButtonClass}>{savingClosure ? "Ajout..." : "Ajouter la fermeture"}</button>
                        </div>
                      </div>
                      <div className={panelClass + " p-5"}>
                        <div className="mb-4 text-lg font-black">Fermetures enregistrées</div>
                        <div className="grid gap-3">
                          {closures.filter((c) => { const now = new Date(); const today = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-${String(now.getDate()).padStart(2,"0")}`; return c.closure_date >= today; }).length === 0 ? (
                            <div className="rounded-[24px] border border-dashed border-[#dccdbb] bg-white/70 px-4 py-5 text-sm text-[#6f6254]">Aucune fermeture exceptionnelle.</div>
                          ) : closures.filter((c) => { const now = new Date(); const today = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-${String(now.getDate()).padStart(2,"0")}`; return c.closure_date >= today; }).map((closure) => (
                            <div key={closure.id} className="rounded-2xl border border-[#eadfce] bg-white p-4 shadow-sm">
                              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                <div>
                                  ...
                                  <div className="mt-1 text-sm text-[#6f6254]">{closure.is_all_day ? "Fermé toute la journée" : `Fermé de ${formatTime(closure.start_time)} à ${formatTime(closure.end_time)}`}</div>
                                  {closure.reason && <div className="mt-1 text-sm text-[#6f6254]">Motif : {closure.reason}</div>}
                                </div>
                                <button type="button" onClick={() => handleDeleteClosure(closure.id)} disabled={deletingClosureId === closure.id} className={dangerButtonClass}>{deletingClosureId === closure.id ? "Suppression..." : "Supprimer"}</button>
                              </div>
                            </div>
                          ))}
                        </div>
                ...
                {activeTab === "categories" && (
                  <div>
                    <div className="mb-6">
                      <div className="mb-2 flex items-center gap-2 text-sm font-bold text-[var(--gold)]"><span className="text-xl">🏷️</span> Catégories</div>
                    ...
                    <div className="grid gap-3">
                      {categories.length === 0 ? (
                        ...
                        <div key={category.id} className="rounded-2xl border border-[#eadfce] bg-[#fffdf9] p-4">
                          <input type="text" defaultValue={category.name} onBlur={(e) => { if (e.target.value !== category.name) handleUpdateCategory(category.id, { name: e.target.value }); }} className="w-full rounded-2xl border border-[#eadfce] bg-white px-4 py-3 font-semibold outline-none transition focus:border-[var(--gold)]" />
                          <div className="mt-4 flex flex-wrap gap-2">
                            {COLOR_OPTIONS.map((color) => {
                              const isSelected = (category.color ?? "#EADCCB") === color;
                              const isUsed = categories.some((c) => c.id !== category.id && c.color === color);
                              return <button key={color} type="button" disabled={isUsed} onClick={() => { if (!isUsed) handleUpdateCategory(category.id, { color }); }} className="h-8 w-8 rounded-full transition hover:scale-105 disabled:cursor-not-allowed disabled:opacity-30" style={{ backgroundColor: color, border: isSelected ? "3px solid #111" : "1px solid #d6d3d1" }} title={isUsed ? "Déjà utilisée" : color} />;
                            })}
                          </div>
                          <div className="mt-4 flex items-center justify-between">
                            ...
                            {confirmDeleteCategoryId === category.id ? (
                              <div className="flex gap-2">
                                <button type="button" onClick={() => { handleDeleteCategory(category.id); setConfirmDeleteCategoryId(null); }} disabled={deletingCategoryId === category.id} className={dangerButtonClass}>{deletingCategoryId === category.id ? "Suppression..." : "Confirmer"}</button>
                                <button type="button" onClick={() => setConfirmDeleteCategoryId(null)} className="rounded-2xl border border-[#eadfce] bg-white px-4 py-2 text-sm font-semibold text-[#6f6254]">Annuler</button>
                              </div>
                            ) : (
                        ...
                        <div className="mt-4 grid gap-2 text-sm font-semibold text-[#6f6254]">
                          ...
                          <div className="flex flex-wrap gap-2 rounded-2xl border border-[#eadfce] bg-white px-4 py-3">
                            {COLOR_OPTIONS.map((color) => {
                              ...
                              return <button key={color} type="button" disabled={isUsed} onClick={() => { if (!isUsed) setNewCategoryColor(color); }} className="h-8 w-8 rounded-full transition hover:scale-105 disabled:cursor-not-allowed disabled:opacity-30" style={{ backgroundColor: color, border: isSelected ? "3px solid #111" : "1px solid #d6d3d1" }} title={isUsed ? "Déjà utilisée" : color} />;
                            })}
                          </div>
                        </div>
                        <button type="button" onClick={handleCreateCategory} disabled={savingCategory} className="mt-5 w-full rounded-2xl bg-[#1f1b17] px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:opacity-90 disabled:opacity-50">{savingCategory ? "Ajout..." : "Ajouter la catégorie"}</button>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {activeTab === "prestations" && (
                  <div>
                    <div className="mb-6">
                      <div className="mb-2 flex items-center gap-2 text-sm font-bold text-[var(--gold)]"><span className="text-xl">✂️</span> Prestations</div>
                      <h2 className="text-2xl font-black tracking-tight">Gérer les prestations</h2>
                    </div>
                    <div className="grid gap-4">
                      <div className="flex flex-wrap gap-2">
                        <button type="button" onClick={() => setSelectedCategory("all")} className={`rounded-2xl px-4 py-2 text-sm font-semibold transition ${selectedCategory === "all" ? "bg-[#1f1b17] text-white" : "border border-[#eadfce] bg-white text-[#4d453d]"}`}>Toutes</button>
                        {categories.map((cat) => (
                          <button key={cat.id} type="button" onClick={() => setSelectedCategory(cat.id)} className={`rounded-2xl px-4 py-2 text-sm font-semibold transition ${selectedCategory === cat.id ? "bg-[#1f1b17] text-white" : "border border-[#eadfce] bg-white text-[#4d453d]"}`}>{cat.name}</button>
                        ))}
                      </div>
                        ...
                        return (
                          <div key={service.id} className={panelClass + " p-4"}>
                            ...
                            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                              <label className="grid gap-2 text-sm font-semibold text-[#6f6254] xl:col-span-2">Nom<input type="text" defaultValue={service.name} onBlur={(e) => { if (e.target.value !== service.name) handleUpdateService(service.id, { name: e.target.value }); }} className={fieldClass} /></label>
                              <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Temps 1 (min)<input type="number" defaultValue={before} onBlur={(e) => { const v = Number(e.target.value)||0; if (v !== before) handleUpdateService(service.id, { duration_before_break: v }); }} className={fieldClass} /></label>
                              <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Pause (min)<input type="number" defaultValue={pause} onBlur={(e) => { const v = Number(e.target.value)||0; if (v !== pause) handleUpdateService(service.id, { break_duration: v }); }} className={fieldClass} /></label>
                              <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Temps 2 (min)<input type="number" defaultValue={after} onBlur={(e) => { const v = Number(e.target.value)||0; if (v !== after) handleUpdateService(service.id, { duration_after_break: v }); }} className={fieldClass} /></label>
                              <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Tarif €<input type="text" defaultValue={(service.price_cents/100).toFixed(2)} onBlur={(e) => { const v = normalizePriceToCents(e.target.value); if (v !== service.price_cents) handleUpdateService(service.id, { price_cents: v }); }} className={fieldClass} /></label>
                              <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Ordre<input type="number" defaultValue={service.display_order} onBlur={(e) => { const v = Number(e.target.value)||0; if (v !== service.display_order) handleUpdateService(service.id, { display_order: v }); }} className={fieldClass} /></label>
                              <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Catégorie
                            ...
                            <div className="mt-4 flex justify-end gap-2">
                              {confirmDeleteServiceId === service.id ? (
                                <>
                                  <button type="button" onClick={() => { handleDeleteService(service.id); setConfirmDeleteServiceId(null); }} disabled={deletingServiceId === service.id} className={dangerButtonClass}>{deletingServiceId === service.id ? "Suppression..." : "Confirmer"}</button>
                                  <button type="button" onClick={() => setConfirmDeleteServiceId(null)} className="rounded-2xl border border-[#eadfce] bg-white px-4 py-2 text-sm font-semibold text-[#6f6254]">Annuler</button>
                                </>
                              ) : (
                      ...
                      <div className="mt-2 rounded-3xl border border-[#eadfce] bg-[#fffdf9] p-5">
                        ...
                        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                          <label className="grid gap-2 text-sm font-semibold text-[#6f6254] xl:col-span-2">Nom<input type="text" value={newServiceName} onChange={(e) => setNewServiceName(e.target.value)} className={fieldClass} /></label>
                          <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Temps 1<input type="number" min={0} value={newServiceBeforeBreak} onChange={(e) => setNewServiceBeforeBreak(e.target.value)} className={fieldClass} /></label>
                          <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Pause<input type="number" min={0} value={newServiceBreakDuration} onChange={(e) => setNewServiceBreakDuration(e.target.value)} className={fieldClass} /></label>
                          <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Temps 2<input type="number" min={0} value={newServiceAfterBreak} onChange={(e) => setNewServiceAfterBreak(e.target.value)} className={fieldClass} /></label>
                          <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Tarif €<input type="text" value={newServicePrice} onChange={(e) => setNewServicePrice(e.target.value)} className={fieldClass} /></label>
                          <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Catégorie
                          ...
                          <label className="grid gap-2 text-sm font-semibold text-[#6f6254]">Ordre<input type="number" value={newServiceOrder} onChange={(e) => setNewServiceOrder(e.target.value)} className={fieldClass} /></label>
                          <label className="flex items-center justify-between rounded-2xl border border-[#eadfce] bg-white px-4 py-3 text-sm font-semibold">
                        ...
                        <div className="mt-5 flex justify-end">
                          <button type="button" onClick={handleCreateService} disabled={savingService} className={primaryButtonClass}>{savingService ? "Ajout..." : "Ajouter la prestation"}</button>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {activeTab === "equipe" && (
                  <div>
                    <div className="mb-6">
                      <div className="mb-2 flex items-center gap-2 text-sm font-bold text-[var(--gold)]"><span className="text-xl">💇</span> Équipe</div>
                      <h2 className="text-2xl font-black tracking-tight">Coiffeuses</h2>
                    </div>
                    <div className="space-y-6">
                            ...
                            <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {confirmDeleteStaffId === member.id ? (
                                  <>
                                    <button type="button" onClick={() => handleDeleteStaff(member.id)} disabled={deletingStaffId === member.id} className={dangerButtonClass}>{deletingStaffId === member.id ? "..." : "Confirmer"}</button>
                                  ...
                                  <div key={dow} className={`rounded-2xl border px-3 py-2 transition ${!salonDayOpen ? "border-dashed border-[#e0d8cf] bg-[#f8f5f1] opacity-60" : sched.is_open ? "border-[#eadfce] bg-white" : "border-[#eadfce] bg-[#faf8f5]"}`}>
                                    <div className="flex flex-wrap items-center gap-3">
                                      <div className="flex items-center gap-2 w-24 shrink-0">
                                        <button type="button" disabled={!salonDayOpen} onClick={() => { if (salonDayOpen) handleUpdateSchedule(sched.id, { is_open: !sched.is_open }); }} className={`h-5 w-9 rounded-full transition-colors ${sched.is_open && salonDayOpen ? "bg-[#1f1b17]" : "bg-[#d8d0c8]"}`}>
                                      ...
                                      {sched.is_open && salonDayOpen && (
                                        <>
                                          <div className="flex items-center gap-1.5">
                                            <input type="time" suppressHydrationWarning defaultValue={sched.opening_time.slice(0,5)} min={salonOpen} max={salonClose} onBlur={(e) => { if (!e.target.value) return; const v = e.target.value; const c = v < salonOpen ? salonOpen : v > salonClose ? salonClose : v; handleUpdateSchedule(sched.id, { opening_time: c }); }} className="rounded-xl border border-[#eadfce] bg-[#faf8f5] px-2 py-1 text-xs outline-none focus:border-[var(--gold)]" />
                                            <span className="text-xs text-[#a09890]">→</span>
                                            <input type="time" suppressHydrationWarning defaultValue={sched.closing_time.slice(0,5)} min={salonOpen} max={salonClose} onBlur={(e) => { if (!e.target.value) return; const v = e.target.value; const c = v < salonOpen ? salonOpen : v > salonClose ? salonClose : v; handleUpdateSchedule(sched.id, { closing_time: c }); }} className="rounded-xl border border-[#eadfce] bg-[#faf8f5] px-2 py-1 text-xs outline-none focus:border-[var(--gold)]" />
                                          </div>
                                          <label className="flex items-center gap-1.5 text-xs text-[#6f6254] cursor-pointer">
                                            <input type="checkbox" checked={sched.has_break} onChange={(e) => handleUpdateSchedule(sched.id, { has_break: e.target.checked, break_start: e.target.checked ? (sched.break_start ?? "12:00") : null, break_end: e.target.checked ? (sched.break_end ?? "14:00") : null })} />
                                            Pause
                                          </label>
                                          {sched.has_break && (
                                            <div className="flex items-center gap-1.5">
                                              <span className="text-xs text-[#a09890]">de</span>
                                              <input type="time" suppressHydrationWarning defaultValue={(sched.break_start ?? "12:00").slice(0,5)} onBlur={(e) => { if (e.target.value) handleUpdateSchedule(sched.id, { break_start: e.target.value }); }} className="rounded-xl border border-[#eadfce] bg-[#fff8ee] px-2 py-1 text-xs outline-none focus:border-[var(--gold)]" />
                                              <span className="text-xs text-[#a09890]">à</span>
                                              <input type="time" suppressHydrationWarning defaultValue={(sched.break_end ?? "14:00").slice(0,5)} onBlur={(e) => { if (e.target.value) handleUpdateSchedule(sched.id, { break_end: e.target.value }); }} className="rounded-xl border border-[#eadfce] bg-[#fff8ee] px-2 py-1 text-xs outline-none focus:border-[var(--gold)]" />
                                            </div>
                                          )}
                                        </>
                                      )}
                            ...
                            <div className="mt-4 grid gap-1">
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                {COLOR_OPTIONS.map((c) => {
                                  ...
                                  return <button key={c} type="button" disabled={takenByOther && !isSelected} onClick={() => { if (!takenByOther || isSelected) handleUpdateStaff(member.id, { color: c }); }} className={`h-7 w-7 rounded-lg border-2 transition ${isSelected ? "border-[#1f1b17] scale-110" : takenByOther ? "cursor-not-allowed opacity-30" : "border-transparent hover:scale-105"}`} style={{ backgroundColor: c }} />;
                                })}
                              </div>
                            </div>
                      ...
                      <div className="rounded-2xl border border-[#eadfce] bg-[#fffdf9] p-4">
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                          <label className="grid gap-1 text-xs font-semibold text-[#6f6254]">Prénom<input type="text" value={newStaffFirstName} onChange={(e) => setNewStaffFirstName(e.target.value)} className={fieldClass} /></label>
                          <label className="grid gap-1 text-xs font-semibold text-[#6f6254]">Nom<input type="text" value={newStaffLastName} onChange={(e) => setNewStaffLastName(e.target.value)} className={fieldClass} /></label>
                        </div>
                        <div className="mt-3 grid gap-1">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {COLOR_OPTIONS.map((c) => {
                              ...
                              return <button key={c} type="button" disabled={taken} onClick={() => { if (!taken) setNewStaffColor(c); }} className={`h-7 w-7 rounded-lg border-2 transition ${newStaffColor === c ? "border-[#1f1b17] scale-110" : taken ? "cursor-not-allowed opacity-30" : "border-transparent hover:scale-105"}`} style={{ backgroundColor: c }} />;
                            })}
                          </div>
                        </div>
                        <p className="mt-2 text-xs text-[#9a9089]">Les horaires par jour seront configurables après la création.</p>
                        <div className="mt-4 flex justify-end">
                          <button type="button" onClick={handleCreateStaff} disabled={savingStaff} className={primaryButtonClass}>{savingStaff ? "Ajout..." : "Ajouter"}</button>
                        </div>
                      </div>
                    </div>
                  </div>
